classdef BoundaryBox_v2
    ...
    methods(Static)
        ...
        while (s(1) > 0)
            ...
            for slice = min_val(sec_plane):32:max_val(sec_plane)+4
                ...
                list = BoundaryBox_v2.path_finding_v5(list, -1);
                ...
                [step_pass, point_list_step] = BoundaryBox_v2.get_trajectory_with_constant_step(current_pass, list, T, x, y, z);
                
                % Moves points along normals to define the path of the
                % actuator (seems like redundant)
                [exp_pass, point_list_exp] = BoundaryBox_v2.expand_trajectory_v3(step_pass, point_list_step, norm, 5);
                
                % Smoothes normals to fix sudden moves of the actuator
                [filtered_norm] = BoundaryBox_v2.norm_filter(exp_pass, step_pass);
                
                % Moves points along smoothed normals to define the path of 
                % the actuator
                [filtered_pass] = BoundaryBox_v2.expand_trajectory_by_norm(step_pass, filtered_norm, 5); 
        ...
        function path_list = path_finding_v5(list, start_point)
        ...
        function [trajectory, point_list] = expand_trajectory_v2(trajectory, point_list, norm, dist)
            
            trajectory = [];
            % Get expanded list of points
            list_size = size(point_list);
            
            list_expanded(list_size(2) * 2 - 2) = PointList();
            ...
            for i = 2:(list_size(2) - 1) * 2
                pts = point_list(floor((i) / 2) + 1).points;
                tris = point_list(floor((i) / 2) + 1).triangles;
                in_tris = point_list(floor((i) / 2) + 1).inside_triangle;
                ...
                if (isempty(list_expanded(i).triangles))
                    ...
                    if mod(i, 2) == 0
                        ...
                        if (length(list_expanded(i).triangles) == 1)
                            list_expanded(i) = list_expanded(i-1);
                            continue;
                        end
                        tri = list_expanded(i).triangles(2);
                    end
                end
%                 if (norm(tri, 3) < 0)
%                     norm(tri, :) = norm(tri, :) * (-1);
...
%             end
            ...
            for i = 1:2:(length(trajectory) - 3)
                dist1 = mathHelper.get_distance(trajectory(i, :), trajectory(i + 1, :));
                dist2 = mathHelper.get_distance(trajectory(i, :), trajectory(i + 2, :));
        ...
        function [trajectory, point_list] = expand_trajectory_v3(trajectory, point_list, norm, dist)
